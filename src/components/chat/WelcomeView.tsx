    ...
    <div className="employee-home">
      ...
      <div className="employee-home-grid">
        ...
        <aside className="employee-activity">
          ...
          {recentActivity.length ? <div className="employee-activity-list">{recentActivity.map((item) => <article key={item.id}><span className={`employee-activity-icon ${item.is_read ? '' : 'new'}`}>{item.type === 'mention' ? '@' : '●'}</span><div><h3>{item.title}</h3><p>{item.message || 'There is new activity in your workspace.'}</p><time>{new Date(item.created_at).toLocaleDateString()}</time></div></article>)}</div> : <div className="employee-activity-empty"><span>✦</span><b>No new activity</b><p>When teammates mention you or share an update, it will appear here.</p></div>}
      ...
      <section className="employee-quick-actions">
        ...
        <div className="employee-quick-grid">
          <button onClick={onStartChat}><i>✦</i><b>Start a chat</b><small>Message a teammate</small></button>
          <button onClick={onBrowseChannels}><i>⌘</i><b>Browse channels</b><small>Explore shared spaces</small></button>
          <button onClick={onSearch}><Icon name="search" size={16} /><b>Search workspace</b><small>Find messages and people</small></button>
        </div>
      </section>
    </div>
  );
